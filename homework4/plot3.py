...
if __name__ == '__main__':
    ...
    MODEL1 = "dailymail_word2vec-skipgram-trained.model"
    #MODEL1 = "dysautonomia_word2vec-cbow-trained.model"
    #MODEL1 = "dysautonomia_word2vec-skipgram-trained.model"
    ...
    #tempfile = './sentences_data/works_list.txt'
    #f = open(tempfile, 'r')
    #read_words = f.read().lower()
    #read_words1= re.sub(r'[^\w\s]','',str(read_words).replace('/', ' '))
    #read_words = read_words.replace('\n', '')
    #list_words = word_tokenize(read_words)
    #print(list_words)
    stop_words = set(stopwords.words('english'))
    
    #tokens = word_tokenize(read_words1.lower())
    ...
    n=0
    for word in words1:
        if word in words1 and not word in vocabs1 and not word in stop_words and n < 5000:
            vocabs1.append(word)
            #print(word)
    ...
    '''
    for word in words2:
        if word in words2 and not word in vocabs1 and not word in stop_words:
            vocabs2.append(word)
            #print(word)
